...
export const TRUST_PAGES: Record<Exclude<PublicPagePath, "/">, TrustPage> = {
  ...
  "/privacy": {
    ...
    body: [
      ...
      "uYao Agent 不要求建立會員或長期健康檔案。原始對話會送至 uYao 伺服器作範圍檢查，但不轉送給外部 AI。啟用 OpenAI 或 Anthropic 時，只交付通過完整目錄詞檢查的標準查詢、選擇的服務區、伺服器整理的查詢指令與公開品項資料，可能涉及境外處理。請勿在聊天輸入姓名、電話、健保、病歷或他人處方。",
  ...
  "/docs": {
    ...
    canonicalPath: "/docs",
    locale: "en",
    title: "uYao Developer Resources and Public API (uyaohealth)",
    kicker: "Docs",
    description:
      "Official uYao 有藥 (uyaohealth.com) developer resources: public read API, OpenAPI, authentication, and CLI source. Not live inventory.",
    body: [
      HOMEPAGE_PROSE,
      `What this site will not claim: ${HOMEPAGE_LIMITS.join(" ")}`,
      "uYao 有藥 (uyaohealth) developer resources live on the canonical domain uyaohealth.com: /docs, /openapi.json, and /llms.txt. The public read contract covers the catalog, individual catalog items, and public pharmacy records. Other routes are not a supported agent API.",
      "Authentication: the public GET endpoints require no API key, account, cookies, or bearer token. Store OS login is separate and does not grant a public automation API. No public MCP server or webhook subscription is published.",
      "uYao CLI (uyaohealth-cli): read-only source at https://github.com/jh1nresh/uyao/tree/main/cli supports catalog, catalog item, pharmacies, and OpenAPI commands. Requires Node.js 20 or newer. The package is not published to npm yet; use the repository installation instructions. No write operations are supported.",
      "GET /api/catalog/{slug} returns one catalog item with its packaging details and listed pharmacies. These are not stock confirmations or medical advice.",
