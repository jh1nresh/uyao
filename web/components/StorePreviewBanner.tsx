  ...
  return (
    <div className="border-b-2 border-green bg-green-tint px-4 py-2.5 sm:px-7 xl:px-12 2xl:px-16">
      <p className="text-[14px] leading-[1.6] text-ink">
        {demo ? locale === "en" ? (
          <><b className="font-bold">DEMO PHARMACY</b> · This synthetic storefront is separate from every real partner pharmacy. Reservations go only to the uYao Store demo inbox and never send LINE notifications.</>
        ) : (
          <><b className="font-bold">示範藥局</b> · 這是與真實合作藥局完全分開的沙盒。預留單只會送到 uYao Store demo inbox，不會發送 LINE 或要求任何真實藥局留貨。</>
        ) : locale === "en" ? (
